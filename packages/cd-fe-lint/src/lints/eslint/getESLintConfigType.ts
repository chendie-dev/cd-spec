 ...
 * @param cwd
 * @param pkg
 * @returns cd-fe-eslint-config/index
 * @returns cd-fe-eslint-config/react
 * @returns cd-fe-eslint-config/typescript/index
 * @returns cd-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'cd-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
